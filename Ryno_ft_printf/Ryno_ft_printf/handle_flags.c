...
/*   Updated: 2021/01/08 17:53:30 by rmeiboom      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_flags(t_flags *flags)
{
	...
	ptr = flags->format;
	i = 0;
	while (strchr(ALL_FLAGS, ptr[i]))
	{
		if (ptr[0] == '0' && flags->is_zero == 0)
			flags->is_zero = 1;
		else if (strchr("csdipxXu", ptr[i]))
			flags->type = ptr[i];
		else if (ptr[i] == '.')
...
int			handle_flags(va_list ap, t_flags *flags)
{
	...
	if (strchr("xXuid", flags->type) && flags->is_precision)
		flags->is_zero = 0;
